...
- **[Pocketsphinx](#pocketsphinx-stt)** is an open-source speech decoder by the [CMU Sphinx](http://en.wikipedia.org/wiki/CMU_Sphinx) project. It's fast and designed to work well on embedded systems (like the Raspberry Pi). Unfortunately, the recognition rate is not the best and it has a lot of dependencies. On the other hand, recognition will be performed offline, i.e. you don't need an active internet connection to use it. It's the right thing to use if you're cautious with your personal data.
